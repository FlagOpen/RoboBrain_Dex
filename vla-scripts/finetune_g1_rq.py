...
import wandb
from Robobrain.models.backbones.llm.prompting import PurePromptBuilder, VicunaV15ChatPromptBuilder
from Robobrain.vla.action_tokenizer import ActionTokenizer
from Robobrain.vla.datasets.rlds.utils.data_utils import save_dataset_statistics
from Robobrain.vla.datasets import RLDSDataset, RLDSQwenVLBatchTransform_withoutCoTLatentAction
...
from Robobrain.extern.hf.modeling_robobrain import OpenVLAForActionPrediction, DexVLAForActionPrediction, ActionDecoder
...
class FinetuneConfig:
    # Directory Paths
    data_root_dir: Path = Path("Your Dataset Directory")            # Path to dataset directory

    vla_path: str = "local Robobrain-Dex path"                      # Path to your local Robobrain-Dex path
    motion_dynamics_path: str = "Motion Dynamics Model Directory"
    motion_dynamics_type: str = "RQ_HandLatentActionModel"

    dataset_name: str = "pour_the_drink"                            # Name of fine-tuning dataset
...
def finetune(cfg: FinetuneConfig) -> None:
    print(f"Fine-tuning Robobrain-Dex Model `{cfg.vla_path}` on `{cfg.dataset_name}`")
    ...
    # Register Robobrain-Dex model to HF Auto Classes (not needed if the model is on HF Hub)
    ...
    # Load Robobrain-Dex Processor and Model using HF AutoClasses
    ...
    vla.action_decoder = ActionDecoder(window_size=cfg.window_size, input_dim=48, vis_dim=2048, hidden_dim=512)
    ...
    def collect_lora_targets(model, include_prefixes=("model.language_model", "model.visual", "lm_head")):
        target_names = []
    ...
    num_warmup_steps = int(cfg.max_steps * 0.03) 
    ...
    def _transform(images):
        out = processor.image_processor.preprocess(images, return_tensors="pt")
        pv = out["pixel_values"]  
    ...
    batch_transform = RLDSQwenVLBatchTransform_withoutCoTLatentAction(
        motion_dynamics_model,
        processor,
        image_transform=_transform,
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            batch["pixel_values"] = batch["pixel_values"].to(device_id)
            batch["image_grid_thw"] = batch["image_grid_thw"].to(device_id)
            ...
            action_logits = vla_output.logits[:, : -1]
            ...
            if gradient_step_idx > 0 and gradient_step_idx % cfg.save_steps == 0:
                ...
                if cfg.use_lora:
                    ...
                    base_vla.action_decoder = ActionDecoder(window_size=cfg.window_size, input_dim=48, vis_dim=2048, hidden_dim=512)
                    ...
                    if cfg.save_latest_checkpoint_only:
                        ...
                        torch.save(vla.module.action_decoder.state_dict(), run_dir / 'action_decoder.pt')
                        print(f"Saved Full Model Checkpoint for Step {gradient_step_idx} at: {run_dir}")
                    else:
                        checkpoint_dir = Path(str(run_dir) + f"--{gradient_step_idx}_chkpt")
                        os.makedirs(checkpoint_dir, exist_ok=True)
                        save_dataset_statistics(vla_dataset.dataset_statistics, checkpoint_dir)
