...
from Robobrain.training.strategies.base_strategy import TrainingStrategy
...
class FSDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        assert isinstance(self.vla, FSDP), "FSDPStrategy.save_checkpoint assumes VLA is already wrapped in FSDP!"
        ...
        with FSDP.state_dict_type(self.vla, self.fsdp_state_dict_type, self.fsdp_save_policy):
            ...
            # Iterate through `full_vla_state_dict` and split `mkey.{full_dotted_path}` -> `mkey: {full_dotted_path}`
            ...
            if overwatch.is_rank_zero():
                ...
                # TODO :: This breaks w/ Sagemaker default permissions (root vs. <user>)... skip?
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        if self.enable_mixed_precision_training and self.mixed_precision_dtype == torch.bfloat16:
            ...
            if self.stage not in {"full-finetune", "vla-full-train", "vla-sandwich-train"}:
                ...
                self.vla.vlm.vision_backbone.to(dtype=self.vla.vlm.vision_backbone.half_precision_dtype)
        ...
        # self.vla.action_decoder = self.vla.action_decoder.to(device=torch.cuda.current_device())
