...
from Robobrain.training.metrics import Metrics, VLAMetrics
from Robobrain.util import check_bloat16_supported
from Robobrain.util.batching_utils import SplitModalitySampler
from Robobrain.util.data_utils import PaddedCollatorForActionPrediction, PaddedCollatorForLanguageModeling
from Robobrain.vla.action_tokenizer import ActionTokenizer
from Robobrain.models.vlas.dexvla import DexVLA_Model
...
class TrainingStrategy(ABC):
    def __init__(
        ...
        **_: str,
        ...
        self.all_module_keys, self.trainable_module_keys = self.vla.all_module_keys, self.vla.vlm.trainable_module_keys + ["action_decoder"]
            ...
            for epoch in range(self.epochs):
                ...
                for train_idx, batch in enumerate(dataloader):
                    ...
                    with torch.autocast(
                        ...
                        output: CausalLMOutputWithPast = self.vla.vlm(
    ...
    def run_vla_training(
        ...
        status = metrics.get_status()
        with tqdm(
            total=(self.epochs * math.ceil(len(dataloader) / overwatch.world_size())) if self.max_steps is None else self.max_steps,
            desc=status,
            leave=False,
            disable=not overwatch.is_rank_zero(),
        ) as progress:
            ...
            for batch in dataloader:
                ...
                with torch.autocast(
                    ...
                    # [Contract] self.vlm.forward() must automatically compute `loss` and return!
                    vla_output, action_loss, loss_one_step, action_tokens, loss_batch = self.vla(batch)
                    la_loss = vla_output.loss
                    loss = la_loss + action_loss
                ...
                action_preds = vla_output.logits[:,  : -1].argmax(dim=2)
                action_gt = batch["labels"][:, 1:].to(action_preds.device)
                if overwatch.is_rank_zero():
                    print('action_preds:', action_preds[0])
                    print("action_gt:", action_gt[0])
               ...
               # Mask out non-special tokens
                ...
                # Commit Metrics
                metrics.commit(action_accuracy=action_accuracy, l1_loss=action_loss, update_step_time=True)
                ...
                if overwatch.is_rank_zero():
                    ...
                    if len(datasets) > 1:
                        for ds in datasets:
                            ...
                            action_l1_loss_ds = loss_batch[ds_mask]
    ...
    def run_latent_action_training(
        ...
        with tqdm(
            ...
            for batch in dataloader:
                ...
                #           => If masking out EOS, then it's just "labels != -100 (IGNORE_INDEX)
                #   3) Compute masked accuracy as `(preds == logits) & mask` --> sum/divide by # unmasked!
                action_preds = output.logits[:, self.vla.vlm.vision_backbone.num_patches : -1].argmax(dim=2)
                action_gt = batch["labels"][:, 1:].to(action_preds.device)
                # Mask out non-special tokens
                mask = action_gt > 32002     
                # mask = action_gt > 50297  
