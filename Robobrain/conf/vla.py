...
model configuration thereof. A given VLA model (`policy`) configures the following attributes:
    ...
    - Base VLM from Robobrain Registry 
...
class VLAConfig(ChoiceRegistry):
    ...
    base_vlm: Union[str, Path]                      # Base VLM as ID/Path to Run Directory 
...
@dataclass
class Exp_DinoSigLIP_224px_Egodex(Exp_SigLIP_224px_Bridge):
    vla_id: str = "prism-dinosiglip-224px+mx-egodex"
    base_vlm: Union[str, Path] = "prism-dinosiglip-224px+7b"

    # data_mix: str = "omni_magic_soup_plus"          # OpenX (Manipulation + Navigation)
    # data_mix: str = "omni_magic_soup_plus_plus"   # OpenX + Humam
    data_mix: str = "egodex"             # lerobot cot
    shuffle_buffer_size: int = 10_000   
    expected_world_size: int = 24
    global_batch_size: int = 768 #16
    per_device_batch_size: int = 32
    epochs: int = 100
    
@dataclass
class Phi_3B_Egodex(Exp_SigLIP_224px_Bridge):
    vla_id: str = "phi-3b-egodex"
    base_vlm: Union[str, Path] = "phi-2+3b"
    ...
    global_batch_size: int = 384 #16
    per_device_batch_size: int = 24
    
# = [32 GPU Pre-training] DINO-SigLIP 224px + Magic Soup++ =
@dataclass
class Exp_DinoSigLIP_224px_Robot(Exp_SigLIP_224px_Bridge):
    vla_id: str = "prism-dinosiglip-224px+mx-robot"
    base_vlm: Union[str, Path] = "prism-dinosiglip-224px+7b"

    # data_mix: str = "omni_magic_soup_plus"          # OpenX (Manipulation + Navigation)
    # data_mix: str = "omni_magic_soup_plus_plus"   # OpenX + Humam
    data_mix: str = "robot"             # lerobot cot

    expected_world_size: int = 8
    global_batch_size: int = 256 #16
    per_device_batch_size: int = 32
    
# = [32 GPU Pre-training] DINO-SigLIP 224px + Magic Soup++ =
@dataclass
class Exp_DinoSigLIP_224px_OXE(Exp_SigLIP_224px_Bridge):
    vla_id: str = "prism-dinosiglip-224px+mx-oxe"
    base_vlm: Union[str, Path] = "prism-dinosiglip-224px+7b"

    # data_mix: str = "omni_magic_soup_plus"          # OpenX (Manipulation + Navigation)
    # data_mix: str = "omni_magic_soup_plus_plus"   # OpenX + Humam
    data_mix: str = "oxe"             # lerobot cot
...
@dataclass
class Exp_Robobrain_224px_Egodex(Exp_SigLIP_224px_Bridge):
    vla_id: str = "robobrain-224px+mx-egodex"
    base_vlm: Union[str, Path] = "qwen2.5+3b"
...
class VLARegistry(Enum):
    ...
    DINOSIGLIP_224PX_MX_EGODEX = Exp_DinoSigLIP_224px_Egodex
    
    # Pre-training on full dataset
    DINOSIGLIP_224PX_MX_EGODEX_3B = Phi_3B_Egodex
    
    # Pre-training on real robot dataset
    DINOSIGLIP_224PX_MX_Robot = Exp_DinoSigLIP_224px_Robot
    
    # Pre-training on OXE dataset
    DINOSIGLIP_224PX_MX_OXE = Exp_DinoSigLIP_224px_OXE
    
    # Pre-training on full dataset
    ROBOBRAIN_224PX_MX_EGODEX = Exp_Robobrain_224px_Egodex
