...
from Robobrain.util.data_utils import tree_map
from Robobrain.vla.action_tokenizer import ActionTokenizer
from Robobrain.vla.constants import ACTION_PROPRIO_NORMALIZATION_TYPE, IGNORE_INDEX
from Robobrain.vla.datasets.rlds import make_interleaved_dataset, make_single_dataset
from Robobrain.vla.datasets.rlds.oxe import OXE_NAMED_MIXTURES, get_oxe_dataset_kwargs_and_weights,get_ego_dataset_kwargs_and_weights
from Robobrain.vla.datasets.rlds.utils.data_utils import NormalizationType
...
class RLDSBatchTransformLIBERO_withHis:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # print(action_vocab)
        action_subtask_tokens = ''
        for i, action in enumerate(action_vocab):
            action_subtask_tokens += action
        
        action_subtask_tokens+=lang

        input_prompt = f"What action should the robot take?"
        if randomized_overlap > 0:
            ...
            input_prompt = "What action should the robot take? History action " + hist_action_tokens

        # Construct Chat-based Prompt =>> Input is default query + language instruction, output are the action tokens
        prompt_builder = self.prompt_builder_fn("openvla")
        conversation = [
            {"from": "human", "value": input_prompt},
            {"from": "gpt", "value": action_subtask_tokens},
        ]
        for turn in conversation:
            prompt_builder.add_turn(turn["from"], turn["value"])
        ...
        #get the latent action start token 
        ...
        # [CRITICAL] We do not want to take the loss for anything but the predicted action tokens!
        # labels[: -(len(action_vocab) +len(action_subtask_tokens)+ 1)] = IGNORE_INDEX
...
@dataclass
class RLDSBatchTransformDex_withHis:
    action_tokenizer: ActionTokenizer
    base_tokenizer: PreTrainedTokenizerBase
    image_transform: ImageTransform
    image_transform_lam: ImageTransform
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with torch.no_grad():
            ...
            video = torch.stack([initial_pixel_values, target_pixel_values], dim=0).unsqueeze(0).to(
                self.action_tokenizer.device)
            latent_action_idx = self.action_tokenizer.vq_encode(video)['indices'].squeeze()
        ...
        # action_subtask_tokens += lang
        action_subtask_tokens += lang.split('\n')[1] + '\n'

        # input_prompt = f"What action should the robot take?"
        input_prompt = lang.split('\n')[0] + '\n'
        if randomized_overlap > 0:
            action_vocab = [f'<ACT_{i.item()}>' for i in hist_action_idx]  # [ACT_1, ACT_2, ... ACT_K]
        ...
        # Tokenize (w/ `base_tokenizer`)
        # print("input:", prompt_builder.get_prompt())
        # print("input_ids:", self.base_tokenizer(prompt_builder.get_prompt(), add_special_tokens=True).input_ids)
        # print("input_out:",  self.base_tokenizer("<ACT_13><ACT_5><ACT_5><ACT_5>i need to pick up the scissors.", add_special_tokens=True).input_ids)
        input_ids = self.base_tokenizer(prompt_builder.get_prompt(), add_special_tokens=True).input_ids
        labels = list(input_ids)
        ...
        if not self.predict_stop_token:
            labels[-1] = IGNORE_INDEX
        episode=rlds_batch['task']['episode_index']
        frame=rlds_batch['task']['frame_index']

        return dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels,
                    actions=rlds_batch["action"][randomized_overlap: self.window_size + randomized_overlap],
                    latent_action_idx=latent_action_idx, dataset_name=dataset_name,episode=episode,frame=frame)
...
@dataclass
class RLDSQwenVLBatchTransform:
    action_tokenizer: ActionTokenizer
    qwen_processor: Any      # Qwen2_5_VLProcessor
    image_transform: ImageTransform
    image_transform_lam: ImageTransform
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        """
        Build Qwen2.5-VL compatible multimodal input.
        """
        """Converts a RLDS batch to the format expected by the OpenVLA collator/models."""
        ...
        pixel_values = pv["pixel_values"]
        image_grid_thw = pv["image_grid_thw"]
        with torch.no_grad():
            initial_pixel_values = self.image_transform_lam(img)
            target_pixel_values = self.image_transform_lam(img_k)
            video = torch.stack([initial_pixel_values, target_pixel_values], dim=0).unsqueeze(0).to(self.action_tokenizer.device)
        ...
        if use_reasoning:
            assistant_output_text = "<BOR>" + reasoning
        else:
            assistant_output_text = action_tokens
        ...
        messages = [
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": f"What action should the robot take to {lang}? "
                                             f"Here is my previous reasoning: {latest_reasoning}."}
                ]
            },
            {
                "role": "assistant",
        ...
        labels[labels == self.qwen_processor.tokenizer.pad_token_id] = -100
        assistant_ids = self.qwen_processor.tokenizer(
            assistant_output_text,
            add_special_tokens=False
        ).input_ids
        labels[:, : -(len(assistant_ids)+5)] = -100
        ...
        # return dict(batch_input, diff_loss_mask=diff_loss_mask)
        return dict(pixel_values=pixel_values, image_grid_thw=image_grid_thw, input_ids=input_ids, labels=labels, dataset_name=dataset_name, proprio=rlds_batch["observation"]["proprio"][0:1], action=rlds_batch["action"][: self.window_size], diff_loss_mask=diff_loss_mask)

@dataclass
class RLDSQwenVLBatchTransform_withoutCoTLatentAction:
    action_tokenizer: ActionTokenizer
    qwen_processor: Any      # Qwen2_5_VLProcessor
    image_transform: ImageTransform
    image_transform_lam: ImageTransform
