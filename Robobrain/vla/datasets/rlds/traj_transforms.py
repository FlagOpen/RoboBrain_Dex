...
def chunk_act_obs_dex(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    """
    ...
    # Create indices for the first and last elements within the window size
    first_indices = tf.range(traj_len)[:, None]  # First index is the current timestep
    last_indices = tf.minimum(first_indices + (window_size + future_action_window_size - 1), goal_timestep[:, None])  # Last index is the end of the window
...
def chunk_act_obs_lerobot(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    """
    Chunks actions and observations into the given window_size.
