...
def     normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    ...
    if normalization_type == NormalizationType.NORMAL:
        ...
        for key, traj_key in keys_to_normalize.items():
            ...
            low = tf.convert_to_tensor(low, dtype=tf.float32)
            high = tf.convert_to_tensor(high, dtype=tf.float32)
            # print("============debug===============")
            # print("traj_key: ",traj_key)
            # print("low: ", low)
            # print("high: ", high)
            traj = dl.transforms.selective_tree_map(
                traj,
...
def get_dataset_statistics(
    ...
    dataset = dataset.traj_map(
        lambda traj: {
            ...
            # "proprio": (
            #     tf.concat([traj["observation"]["EEF_state"],
            #                traj["observation"]["gripper_state"]],
            #               axis=1)
            # ),
