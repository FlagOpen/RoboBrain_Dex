...
from Robobrain.vla.datasets.rlds import obs_transforms, traj_transforms
from Robobrain.vla.datasets.rlds.utils import goal_relabeling, task_augmentation
...
datasets_with_lower_frequency = ['toto', 'berkeley_autolab_ur5',#'bridge_oxe' 
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        new_obs = {}
        # print("traj keys: ",traj["observation"].items())
        # print("image keys:", image_obs_keys.items())
        ...
        #add eef state and index
        # new_obs["EEF_state"] = traj["observation"]["EEF_state"]
        # new_obs["gripper_state"] = traj["observation"]["gripper_width"]
        # new_obs["episode_index"]=traj["observation"]["episode_index"]
        # new_obs["frame_index"] = traj["observation"]["frame_idx"]
        # new_obs["time_stamp"] = traj["observation"]["time_stamp"]
        # new_obs["start_reasoning_index"] = traj["observation"]["start_reasoning_index"]
        # new_obs["end_reasoning_index"] = traj["observation"]["end_reasoning_index"]
...
def make_dataset_from_cot_rlds(
    name: str,
    data_dir: str,
    *,
    train: bool,
    ...
    subdirs = [d for d in tf.io.gfile.listdir(parent_dir)
           if tf.io.gfile.isdir(os.path.join(parent_dir, d))]
    if not subdirs:
        raise ValueError(f"empty Dir:{parent_dir}")
    def make_restructure(ds_name, reasoning_dict):
        def restructure(traj):
            # apply a standardization function, if provided
            if standardize_fn is not None:
                traj = standardize_fn(traj)
            ...
            # add timestep info
            new_obs["timestep"] = tf.range(traj_len)
            new_obs["latest_reasoning"] = traj["observation"]["latest_reasoning"]
            #add eef state and index
            # new_obs["EEF_state"] = traj["observation"]["EEF_state"]
            # new_obs["gripper_state"] = traj["observation"]["gripper_width"]
            new_obs["episode_idx"]=traj["episode_idx"]
            new_obs["frame_idx"] = traj["observation"]["frame_idx"]
            # new_obs["time_stamp"] = traj["observation"]["time_stamp"]
            # new_obs["start_reasoning_index"] = traj["observation"]["start_reasoning_index"]
            # new_obs["end_reasoning_index"] = traj["observation"]["end_reasoning_index"]
            def reasoning_for_traj(ep_idx_tensor, frame_idx_tensor):
                if len(ep_idx_tensor) == 0:
                    return None
                
                # episode index: take the first element if it's per-frame repeated
                ep_idx_np = np.array(ep_idx_tensor)
                if ep_idx_np.ndim > 0:
                    ep_val = int(ep_idx_np[0])
                else:
                    ep_val = int(ep_idx_np.item())
                frames = np.array(frame_idx_tensor).astype(np.int64).reshape(-1) 
                out = np.empty(frames.shape[0], dtype=object)                 
                out[:]  = b"" 

                epi = reasoning_dict.get(str(ep_val), None)
                if epi is None:
                    return out  # no annotations for this episode

                segments = epi.get("reasoning_segments", [])
                for seg in segments:
                    s = int(seg.get("start_frame", 0))
                    e = int(seg.get("end_frame", -1))
                    txt = seg.get("reasoning", "") or ""
                    mask = (frames >= s) & (frames <= e)
                    out[mask] = txt
                return out

            if "frame_idx" in new_obs and "episode_idx" in traj:
                reasoning_text = tf.py_function(
                    func=reasoning_for_traj,
                    inp=[traj["episode_idx"], new_obs["frame_idx"]],
                    Tout=tf.string,
                )
                # Set shape for tf graph friendliness
                reasoning_text.set_shape((None,))
                new_obs["reasoning"] = reasoning_text
            # extracts `language_key` into the "task" dict
            task = {}
            if language_key is not None:
                if traj[language_key].dtype != tf.string:
    ...
    # builder = tfds.builder(name, data_dir=data_dir)
    # #load reasoning json
    # reasoning_path = os.path.join(data_dir, name, "reasoning.json")
    # with tf.io.gfile.GFile(reasoning_path, "r") as f:
    ...
    if isinstance(dataset_statistics, str):
        ...
        for sub in subdirs:
            ...
            builder = tfds.builder(sub, data_dir=parent_dir)
            reasoning_path = os.path.join(parent_dir, sub, "reasoning.json")
            with tf.io.gfile.GFile(reasoning_path, "r") as f:
                reasoning_dict = json.load(f)
            restructure_sub = make_restructure(sub, reasoning_dict)
            sub_dataset = dl.DLataset.from_rlds(
                builder, split="all", shuffle=False, num_parallel_reads=1
            ).traj_map(restructure_sub, num_parallel_calls)
            full_dataset = sub_dataset if full_dataset is None else full_dataset.concatenate(sub_dataset)
        # full_dataset = dl.DLataset.from_rlds(
        #     builder, split="all", shuffle=False, num_parallel_reads=num_parallel_reads
        # ).traj_map(restructure, num_parallel_calls)
        # # tries to load from cache, otherwise computes on the fly
    ...
    for sub in subdirs:
        ...
        if 'ego4d' in name:
            split = "val"
        sub_dataset = dl.DLataset.from_rlds(builder, split=split, shuffle=shuffle, num_parallel_reads=num_parallel_reads)
        reasoning_path = os.path.join(parent_dir, sub, "reasoning.json")
        with tf.io.gfile.GFile(reasoning_path, "r") as f:
            reasoning_dict = json.load(f)
        restructure_sub = make_restructure(sub, reasoning_dict)

        sub_dataset = sub_dataset.traj_map(restructure_sub, num_parallel_calls)
        sub_dataset = sub_dataset.traj_map(
            partial(
                normalize_action_and_proprio, 
                metadata=dataset_statistics,
                normalization_type=action_proprio_normalization_type,
        ...
        dataset = sub_dataset if dataset is None else dataset.concatenate(sub_dataset)
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    if training_phase == 'post-training':
        # transform = traj_transforms.chunk_act_obs_libero
        transform = traj_transforms.chunk_act_obs_dex# load all obs. within a window
    else:       
        transform = traj_transforms.chunk_act_obs_dex           # only load the first and last obs. within a window
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        _, dataset_statistics = make_dataset_from_cot_rlds(**data_kwargs, train=train)
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset, _ = make_dataset_from_cot_rlds(
            **dataset_kwargs,
            train=train,
