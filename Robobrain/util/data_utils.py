...
def add_trailing_padding(tensor, padding_value, num_padding):
    """Append a fixed number of padding values to the right side of each sequence"""
    batch_size, orig_len = tensor.shape
    new_tensor = torch.full((batch_size, orig_len + num_padding), padding_value, dtype=tensor.dtype)
    new_tensor[:, :orig_len] = tensor
    return new_tensor
...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        input_ids = pad_sequence(input_ids, batch_first=True, padding_value=self.pad_token_id)
        labels = pad_sequence(labels, batch_first=True, padding_value=IGNORE_INDEX)
        input_ids = add_trailing_padding(input_ids, self.pad_token_id, self.num_padding)
        labels = add_trailing_padding(labels, IGNORE_INDEX, self.num_padding)
        # Truncate (if necessary)
        input_ids, labels = input_ids[:, : self.model_max_length], labels[:, : self.model_max_length]
        ...
        actions = [torch.from_numpy(instance["action"].copy()) for instance in instances]
        actions = torch.stack(actions, dim=0)
        proprio = [torch.from_numpy(instance["proprio"].copy()) for instance in instances]
        proprio = torch.stack(proprio, dim=0)
        diff_loss_mask = [torch.as_tensor(instance.get("diff_loss_mask", False)).clone().bool() for instance in instances]
        diff_loss_mask = torch.stack(diff_loss_mask, dim=0)
...
class PaddedCollatorForActionPrediction_LIBERO:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels"))
        episode_index, frame_index = tuple([instance[key] for instance in instances] for key in ("episode", "frame"))
        pixel_values = [instance["pixel_values"] for instance in instances]
        if "dataset_name" in instances[0]:
            dataset_names = [instance["dataset_name"] for instance in instances]
        else:
        ...
        # For low-level policy training
        actions = [torch.from_numpy(instance["action"]) for instance in instances]
        actions = torch.stack(actions, dim=0)
