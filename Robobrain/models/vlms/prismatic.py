...
class PrismaticVLM(VLM):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if "vlm.llm_backbone" in model_state_dict:
            assert (
                "vlm.projector" in model_state_dict and "vlm.llm_backbone" in model_state_dict
            ), "PrismaticVLM `from_pretrained` expects checkpoint with keys for `vlm.projector` AND `vlm.llm_backbone`!"

            vlm.projector.load_state_dict(model_state_dict["vlm.projector"])
            vlm.llm_backbone.load_state_dict(model_state_dict["vlm.llm_backbone"])
            if "vlm.vision_backbone" in model_state_dict.keys():
                vlm.vision_backbone.load_state_dict(model_state_dict["vlm.vision_backbone"])
        else:
            assert (
    ...
    def forward(
        ...
        projected_patch_embeddings = self.projector(patch_features) # [bsz, 256, 4096]
        ...
        input_embeddings = self.llm_backbone.embed_input_ids(input_ids) # [bsz, seq_len, 4096]
