...
from Robobrain.models.materialize import get_llm_backbone_and_tokenizer, get_vision_backbone_and_transform
from Robobrain.models.registry import GLOBAL_REGISTRY, MODEL_REGISTRY
from Robobrain.models.vlas import OpenVLA
from Robobrain.models.vlms import PrismaticVLM, QwenVLVLM
...
def load(
    ...
    overwatch.info(
        f"Found Config =>> Loading & Freezing [bold blue]{model_cfg['architectures']}[/] with:\n"
        f"             Vision Backbone =>> [bold]{model_cfg['model_type']}[/]\n"
        f"             LLM Backbone    =>> [bold]{model_cfg['model_type']}[/]\n"
        f"             Arch Specifier  =>> [bold]{model_cfg['architectures']}[/]\n"
    )

    # Load VLM using `from_pretrained` (clobbers HF syntax... eventually should reconcile)
    overwatch.info(f"Loading VLM [bold blue]{model_cfg['model_type']}[/] from Checkpoint")
    vlm = QwenVLVLM(model_id=model_id_or_path)
...
def load_vla(
    ...
    # TODO :: Unify semantics with `load()` above; right now, `load_vla()` assumes path points to
    ...
    # llm_backbone.llm.resize_token_embeddings(32097)

    # Load VLM using `from_pretrained` (clobbers HF syntax... eventually should reconcile)
    overwatch.info(f"Loading VLA [bold blue]{model_cfg.model_id}[/] from Checkpoint")
    vla = DexVLA_Model.from_pretrained(
        checkpoint_pt,
        cache_dir,
